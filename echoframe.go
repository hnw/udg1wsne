...
func ParseEchoFrame(raw []byte) (*echoFrame, error) {
	...
	if binary.BigEndian.Uint16(raw[0:2]) != EHD_EchonetLite {
		return nil, errors.New("Unknown ECHONET Lite Header")
	}
	// トランザクションID
...
func (self *echoFrame) Build() []byte {
	...
	binary.Write(buf, binary.BigEndian, uint16(EHD_EchonetLite))
	// トランザクションID
